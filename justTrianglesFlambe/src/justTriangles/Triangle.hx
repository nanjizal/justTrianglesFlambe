...
class Triangle {
    ...
    public function rescaleAbove0( dx: Float, dy: Float, sx: Float, sy: Float ){
        moveDelta( 1, 1 );
        // change to be above 0
        var sx0 = sx/2;
        var sy0 = sy/2;
        // change scale to be correct
        ax *= sx0;
        bx *= sx0;
        cx *= sx0;
        ay *= sy0;
    ...
    public function drawStrips( drawRect: Float->Float->Float->Float->Void ){
        ...
        if( xi == righti ) righti = xi+ 1;
        if( yi == bottomi ) bottomi = yi + 1;
        var sx: Int = 0;
        var ex: Int = 0;
        var sFound: Bool;
        var eFound: Bool;
