...
class Main {
    ...
    static var rainbow = [ Black, Red, Orange, Yellow, Green, Blue, Indigo, Violet, White ];
    ...
    static function draw(){
        ...
        ctx.setThickness( 10 );
        ctx.setColor( 2, 2 );
        ctx.lineType = TriangleJoinStraight; // - lighter 
        var pathTrace = new PathContextTrace();
        var p = new SvgPath( ctx );
        p.parse( bird_d, 0, 0 );
        ctx.render( thick, false ); 
    }
    static public function renderToTriangles(){
        var tri: Triangle;
        ...
        for( i in 0...triangles.length ){
            tri = triangles[ i ];
            tri.rescaleAbove0( o, o, s, s );
            fillColor = cast rainbow[ tri.colorID ];
